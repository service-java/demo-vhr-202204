...
@Controller
@Api(tags="Employee")
@RequestMapping(EruptRestPath.ERUPT_API + "/employee")
@Log
public class EmployeeController {
    ...
    @ApiOperation("pageEmployee")
    @GetMapping("/pageEmployee")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageEmployee(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(employeeMapper.pageEmployee(PageDtoUtil.convert(pageQueryVo)));
    }
