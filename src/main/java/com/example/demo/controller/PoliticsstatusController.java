...
import com.example.demo.entity.Politicsstatus;
...
@RequestMapping(EruptRestPath.ERUPT_API + "/politicsstatus")
@Log
public class PoliticsstatusController {
    ...
    @ApiOperation("pagePoliticsstatus")
    @GetMapping("/pagePoliticsstatus")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pagePoliticsstatus(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(politicsstatusMapper.pagePoliticsstatus(PageDtoUtil.convert(pageQueryVo)));
    }
