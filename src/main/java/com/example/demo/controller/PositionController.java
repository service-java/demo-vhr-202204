...
@Controller
@Api(tags="Position")
@RequestMapping(EruptRestPath.ERUPT_API + "/position")
@Log
public class PositionController {
    ...
    @ApiOperation("pagePosition")
    @GetMapping("/pagePosition")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pagePosition(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(positionMapper.pagePosition(PageDtoUtil.convert(pageQueryVo)));
    }
