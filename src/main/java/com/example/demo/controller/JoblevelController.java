...
@Controller
@Api(tags="Joblevel")
@RequestMapping(EruptRestPath.ERUPT_API + "/joblevel")
@Log
public class JoblevelController {
    ...
    @ApiOperation("pageJoblevel")
    @GetMapping("/pageJoblevel")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageJoblevel(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(joblevelMapper.pageJoblevel(PageDtoUtil.convert(pageQueryVo)));
    }
