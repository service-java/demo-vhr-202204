...
@Controller
@Api(tags="MenuRole")
@RequestMapping(EruptRestPath.ERUPT_API + "/menuRole")
@Log
public class MenuRoleController {
    ...
    @ApiOperation("pageMenuRole")
    @GetMapping("/pageMenuRole")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageMenuRole(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(menuRoleMapper.pageMenuRole(PageDtoUtil.convert(pageQueryVo)));
    }
