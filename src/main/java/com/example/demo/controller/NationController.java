...
@Controller
@Api(tags="Nation")
@RequestMapping(EruptRestPath.ERUPT_API + "/nation")
@Log
public class NationController {
    ...
    @ApiOperation("pageNation")
    @GetMapping("/pageNation")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageNation(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(nationMapper.pageNation(PageDtoUtil.convert(pageQueryVo)));
    }
