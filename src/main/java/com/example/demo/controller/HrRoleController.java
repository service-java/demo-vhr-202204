...
import com.example.demo.entity.HrRole;
...
@Controller
@Api(tags="HrRole")
@RequestMapping(EruptRestPath.ERUPT_API + "/hrRole")
@Log
public class HrRoleController {
    ...
    @ApiOperation("pageHrRole")
    @GetMapping("/pageHrRole")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageHrRole(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(hrRoleMapper.pageHrRole(PageDtoUtil.convert(pageQueryVo)));
    }
