...
@Controller
@Api(tags="Menu")
@RequestMapping(EruptRestPath.ERUPT_API + "/menu")
@Log
public class MenuController {
    ...
    @ApiOperation("pageMenu")
    @GetMapping("/pageMenu")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageMenu(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(menuMapper.pageMenu(PageDtoUtil.convert(pageQueryVo)));
    }
