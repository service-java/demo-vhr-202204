...
@Controller
@Api(tags="MailSendLog")
@RequestMapping(EruptRestPath.ERUPT_API + "/mailSendLog")
@Log
public class MailSendLogController {
    ...
    @ApiOperation("pageMailSendLog")
    @GetMapping("/pageMailSendLog")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageMailSendLog(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(mailSendLogMapper.pageMailSendLog(PageDtoUtil.convert(pageQueryVo)));
    }
