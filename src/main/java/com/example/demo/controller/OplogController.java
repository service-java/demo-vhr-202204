...
@Controller
@Api(tags="Oplog")
@RequestMapping(EruptRestPath.ERUPT_API + "/oplog")
@Log
public class OplogController {
    ...
    @ApiOperation("pageOplog")
    @GetMapping("/pageOplog")
    @ResponseBody
    // @EruptRecordOperate("登录可调用")
    @EruptRouter(verifyType = EruptRouter.VerifyType.LOGIN, authIndex = 0)
    public ResponseVo pageOplog(PageQueryVo pageQueryVo) {
        ValidatorUtil.validateEntity(pageQueryVo);
        return ResponseVo.ok(oplogMapper.pageOplog(PageDtoUtil.convert(pageQueryVo)));
    }
